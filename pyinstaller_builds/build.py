#!/usr/bin/env python3
"""
Build script for PyCurveBug (Pyglet Edition) using PyInstaller
...
def find_icon(platform_name, custom_icon=None):
    ...
    if custom_icon:
        if os.path.exists(custom_icon):
            ...
            print(f"Custom icon not found: {custom_icon}")
...
app = BUNDLE(
    ...
    {icon_line},
    bundle_identifier='{settings.get('bundle_identifier', 'com.comtek.pycurvebug')}',
...
def create_distribution_package(platform_name):
    ...
    if os.path.exists(BuildConfig.CONFIG_FILE):
        shutil.copy2(BuildConfig.CONFIG_FILE, dist_path / "curvebug_config.json")

    # Create README
    readme_content = f"""# {BuildConfig.APP_NAME} v{BuildConfig.VERSION} (Pyglet Edition)

## About

PyCurveBug is a curve viewer application for vintageTEK CurveBug hardware.
This is the Pyglet edition with minimal dependencies.
...
The application uses `curvebug_config.json` for configuration.
...
- Drag: Pan the view (in fixed scale mode)
- Mouse Wheel: Zoom in/out (in fixed scale mode)
...
This build uses Pyglet for graphics (no pygame or numpy required).
All dependencies are bundled in the executable.

## Troubleshooting

### Serial Port Issues
- Windows: Use Device Manager to find the COM port
- Linux: Check /dev/ttyUSB* or /dev/ttyACM*
- macOS: Check /dev/cu.usbserial* or /dev/cu.usbmodem*

### Display Issues
- Try adjusting window size in settings
- Toggle between auto-scale and fixed scale modes
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Build PyCurveBug (Pyglet Edition) executable')
    ...
    if args.version:
        BuildConfig.VERSION = args.version

    print("=" * 80)
    print(f"Building {BuildConfig.APP_NAME} v{BuildConfig.VERSION} (Pyglet Edition)")
    print("=" * 80)

    # Check if source file exists
    if not os.path.exists(BuildConfig.MAIN_SCRIPT):
        print(f"Error: {BuildConfig.MAIN_SCRIPT} not found!")
        print(f"   Make sure you're running this from the pyinstaller_builds directory")
        return 1
